...
Purpose: help an AI agent be immediately productive editing or extending this small Jupyter/PyCairo project that generates multi-scale Truchet tiles and images.
  ...
  - This repo implements multiscale Truchet tile generation and rendering using PyCairo and NumPy. Core pieces:
    - `src/truchet_viewer/tiler.py` — orchestrates tiling, multiscale splitting, and image-driven tiling (`multiscale_truchet`, `image_truchet`, `show_tiles`).
  ...
  - Quick lint/build: this is a small library — install in development mode with `pip install -e .` or `pip install -e ".[dev]"` for development dependencies. Uses `ruff` for linting. Tests: none present.
